...
namespace Frontend.Services
{
    public class UtilizadorService
    {
        ...
        public async Task<bool> IsAdminAsync()
        {
            ...
            var isAdmin = await _httpClient.GetFromJsonAsync<bool>($"api/utilizador/isadmin/{userId}");
        ...
        public async Task<string> UpdateUtilizadorAsync(int id, UpdateUtilizadorDTO updateUtilizadorDTO)
        {
            try
            {
                var response = await _httpClient.PutAsJsonAsync($"api/utilizador/{id}", updateUtilizadorDTO);
        ...
        public async Task<bool> UpdateUtilizadorAdminAsync(int id, UpdateUtilizadorAdminDTO updateUtilizadorDTO)
        {
            try
            {
                var response = await _httpClient.PutAsJsonAsync($"api/utilizador/admin/{id}", updateUtilizadorDTO);
        ...
        public async Task<UtilizadorDTO> GetUserByIdAsync(int id)
        {
            try
            {
                var utilizador = await _httpClient.GetFromJsonAsync<UtilizadorDTO>($"api/utilizador/{id}");
                return utilizador;
            }
        ...
        public async Task<List<UtilizadorDTO>> GetAllUsersAsync()
        {
            try
            {
                var utilizadores = await _httpClient.GetFromJsonAsync<List<UtilizadorDTO>>("api/utilizador/index");
                return utilizadores;
            }
            catch (HttpRequestException ex)
            {
        ...
        public async Task<string> CreateUtilizadorAsync(CreateUtilizadorDTO createUtilizadorDTO)
        {
            try
            {
                var response = await _httpClient.PostAsJsonAsync("api/Utilizador/create",
                    createUtilizadorDTO);
